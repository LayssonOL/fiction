...
class write_nmls_layout_impl
{
  ...
  private:
    ...
    //             }
    //
    //             if (const auto it = nmls::COMPONENT_SELECTOR.find(type); it != nmls::COMPONENT_SELECTOR.end())
    //             {
    //                 os << fmt::format(nmls::OPEN_LAYOUT_ITEM, it->second, bb_x(c), bb_y(c));
    //             }
    //             else
    //             {
    ...
    //             }
    //
    //             os << nmls::CLOSE_LAYOUT_ITEM;
    //         }
    //     }
    //     os << nmls::CLOSE_LAYOUT;
    // }
};
